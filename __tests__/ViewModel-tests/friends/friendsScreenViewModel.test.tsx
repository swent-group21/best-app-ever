...
describe("useFriendsScreenViewModel", () => {
  ...
  it("fetches and sets users correctly", async () => {
    ...
    jest.spyOn(GetFirestoreCtrl, "getAllUsers").mockImplementationOnce(
      (): Promise<any> =>
      Promise.resolve(
        [{"name": "John Doe", "uid": "user1"}, {"name": "Jane Smith", "uid": "user2"}]
      )
    )
    ...
    expect(result.current.users).toEqual([{"name": "John Doe", "uid": "user1"}, {"name": "Jane Smith", "uid": "user2"}]);
  });
  ...
  it("fetches and sets friends correctly", async () => {
    ...
    jest.spyOn(GetFirestoreCtrl, "getFriends").mockImplementationOnce(
      (): Promise<any> =>
  ...
  it("fetches and sets friend requests correctly", async () => {
    jest.spyOn(GetFirestoreCtrl, "getFriendRequests").mockImplementationOnce(
      (): Promise<any> =>
      Promise.resolve([{ uid: "request1", name: "Bob" }])
    )
