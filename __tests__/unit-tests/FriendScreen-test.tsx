...
jest.mock('@/components/theme/ThemedView', () => ({
  ...
  jest.mock('@/components/friends/ListOfFilteredUsers', () => ({
    __esModule: true,
    default: ({ searchText }: any) => <div data-testid="filtered-users">{searchText}</div>,
  }));
  
  jest.mock('@/components/friends/ListOfFriends', () => ({
    __esModule: true,
    default: ({ friends }: any) => (
      <div data-testid="friends-list">{friends.map((friend: any) => friend.name).join(', ')}</div>
    ),
  }));
  ...
  jest.mock('@/components/friends/RequestsList', () => ({
    ...
    default: (requests: any) => (
      <div data-testid="requests-list">{requests.map((request: any) => request.name).join(', ')}</div>
    ),
  }));
...
describe('FriendsScreen', () => {
  ...
  it('displays friends when fetched', async () => {
    const friendsMock = [{ uid: '1', name: 'Friend 1' }, { uid: '2', name: 'Friend 2' }];
