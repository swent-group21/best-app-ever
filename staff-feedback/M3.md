...
- The late development of complete features led to an app that could have had additional features, but is lacking them.
- Impressive remontada nonetheless.
- Some feature seem kind of light, it feels like the map could have held an even more important place (with route to challenges location for instance).
...
When clicking on the map from a group, the map loads indefinitely when there are no posts in the group.
Clicking on "update email" in the profile screen simply shows (alert) "Email Updated".
When log-in with incorrect credentials, we get an alert saying "Firebase Error", which is not super user-friendly and leaks implementation details.
...
- Having errors display the actual error message, instead of a more user-friendly one, feels a bit weird from an end-user point of view.
- The challenge button at the bootom of the group screen is not clickable.
...
- Good improvements in code quality. However there is still an issue with the naming of your files, you have to decide on a convention.
- App architecture and overall design is way better than it was better, but is still not "perfect" (e.g., `FirestoreCtrl` that was split weirdly).
- Figma is up-to-date with the current state of the app
...
It took some time but you finally managed to have a working repository setup and CI pipeline, with tests yielding an important code coverage.
...
- Your "Walkthrough" is nice but lacks technical documentation that would help another developer to quickly take on the project.
- Not sure what are the cells in between the two layers in your architecture diagram.
...
You were fully autonomous by the end of the milestone. Our intervention was no longer needed during the meetings and you knew what you had to do in the last few sprints.
...
Based on the above points, your intermediate grade for this milestone M3 is 5.11.
